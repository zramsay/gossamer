...
func (s *Service) isCompletable() (bool, error) {
	...
	if uint64(s.lenVotes(models.Precommit)+len(s.pcEquivocations)) <= s.state.Threshold() {
		return false, nil
	}
	...
	votes := s.getVotes(models.Precommit)
	...
	for _, v := range votes {
		...
		c, err := s.getTotalVotesForBlock(v.Hash, models.Precommit)
		if err != nil {
			return false, err
		}
