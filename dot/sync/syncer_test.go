...
func newMockFinalityGadget(t *testing.T) *mocks.FinalityGadget {
	m := mocks.NewFinalityGadget(t)
...
func newMockBabeVerifier(t *testing.T) *mocks.BabeVerifier {
	m := mocks.NewBabeVerifier(t)
	m.On("VerifyBlock", mock.AnythingOfType("*types.Header")).Return(nil)
	return m
}

func newMockNetwork(t *testing.T) *mocks.Network {
	m := mocks.NewNetwork(t)
...
func newTestSyncer(t *testing.T) *Service {
	...
	cfg.BabeVerifier = newMockBabeVerifier(t)
	cfg.LogLvl = log.Trace
	cfg.FinalityGadget = newMockFinalityGadget(t)
