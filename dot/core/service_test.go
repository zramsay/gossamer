...
func Test_Service_maintainTransactionPool(t *testing.T) {
	...
	t.Run("Validate Transaction err", func(t *testing.T) {
		...
		mockStorageState.EXPECT().Lock()
		mockStorageState.EXPECT().Unlock()
		mockStorageState.EXPECT().TrieState(nil).Return(ts, nil)
		service = &Service{
...
func Test_Service_handleBlocksAsync(t *testing.T) {
	...
	t.Run("handleChainReorg error", func(t *testing.T) {
		...
		mockTxnStateErr.EXPECT().RemoveExtrinsic(types.Extrinsic{21}).Times(1)
...
func TestService_handleChainReorg(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		...
		runtimeMockOk.On("Version").Return(runtimeVersionData, nil)
